...
typedef struct axCOLORKEY_T {
    AX_U16 u16Enable;
    AX_U16 u16Inv;      /* RW; 0: winin threshold, 1: outside threshold */
    AX_U32 u32KeyLow;   /* RW; min value of color key 0xRRGGBB */
    AX_U32 u32KeyHigh;  /* RW; max value of color key; 0xRRGGBB */
} AX_COLORKEY_T;

typedef struct
{
    AX_U32   nColor;        /* RW; range: [0, 0xffffff]; color RGB888; 0xRRGGBB */
    AX_BOOL  bColorInvEn;   /* RW; range: [0, 1]; whether use background color inv or not */
